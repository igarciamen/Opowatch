...
public class UserService {
    ...
    public User updateSubscription(String username, boolean subscribed) {
        User user = findByUsername(username);
        user.setSubscribedToNotifications(subscribed);
        return userRepo.save(user);
    }

    public List<String> findSubscriberEmails() {
        return userRepo.findBySubscribedToNotificationsTrue()
                .stream()
                .map(User::getEmail)
                .toList();
    }
