...
describe('AuthGuard', () => {
  ...
  it('con el rol requerido: permite', (done) => {
    const user: UserInfo = {
      id: 1, username: 'isabel', email: 'isabel@admin.local',
      roles: ['ROLE_ADMIN'], subscribedToNotifications: true
    };
    const auth = {
  ...
  it('sin el rol requerido: deniega y va a /', (done) => {
    const user: UserInfo = {
      id: 2, username: 'marco', email: 'marco@mail.com',
      roles: ['ROLE_USER'], subscribedToNotifications: true
    };
    const auth = {
      isAuthenticated: () => true,
      getRoles: () => ['ROLE_USER'],
    ...
    guard.canActivate(routeWith(['ROLE_ADMIN']), state).subscribe((ok) => {
      ...
      console.log('Permite?', ok, '-> redirige a /');
      done();
    });
  });
});