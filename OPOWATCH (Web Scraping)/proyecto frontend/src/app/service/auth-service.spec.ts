...
describe('AuthService', () => {
  ...
  it('hasRole y getRoles reflejan los roles del usuario', (done) => {
    const fakeUser: UserInfo = {
      id: 2, username: 'marco', email: 'marco@mail.com', roles: ['ROLE_SELLER'],
    };

    service.login('marco', '123456').subscribe(() => {
      expect(service.getRoles()).toEqual(['ROLE_SELLER']);
      expect(service.hasRole('ROLE_SELLER')).toBe(true);
      ...
      console.log('hasRole(ROLE_SELLER):', service.hasRole('ROLE_SELLER'));
